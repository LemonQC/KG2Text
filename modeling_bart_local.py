# PyTorch BART model
# This code is modified based on modeling_bart.py in Huggingface Transformers and JointGT (https://github.com/thu-coai/JointGT/blob/main/modeling_bart.py).
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        self.embed_tokens = embed_tokens
        self.fusion_module=nn.Linear(config.d_model*2,config.d_model)
        self.init_fusion()
    ...
    def init_fusion(self):
        nn.init.xavier_normal_(self.fusion_module.weight)
        nn.init.constant_(self.fusion_module.bias, 0)
    def forward(
        self,
        input_ids,
        encoder_hidden_states,
        ...
        **unused,
        ...
        if use_pos is not None:
            ...
            # pos_embedding_input=use_pos.unsqueeze(1)
            if not is_training:
                use_pos=use_pos.unsqueeze(1)
            #     use_pos=use_pos.repeat(1,x.shape[1],1)
            # else:
                use_pos=use_pos.repeat(int(x.shape[0]/use_pos.shape[0]),1,1)
            # if use_pos.shape
            new_hidden=torch.cat([x,use_pos],dim=-1)
            x0=self.fusion_module(new_hidden)

            # x0=self.layernorm_embedding(x0)
            x+=x0 *0.001 
            # x=x
            # x=x0+x      
        x += positions
...
class BartStructureModel(PretrainedBartModel):
    ...
    def forward(
        ...
        assert isinstance(decoder_outputs[0], torch.Tensor)
        encoder_outputs: Tuple = _filter_out_falsey_values(encoder_outputs)
        return decoder_outputs + encoder_outputs,decoder_outputs[0]
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def prepare_inputs_for_generation(self, decoder_input_ids, past, attention_mask, use_cache, use_pos,is_training,**kwargs):
...
class MyBartForConditionalGeneration(BartForConditionalGeneration):
    def __init__(self, config):
        ...
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        #下面是进行copy paste
        self.proj_linear=nn.Linear(self.model.shared.embedding_dim*3,1)
        self.coefficient = torch.nn.Parameter(torch.Tensor([0.2]))
        self.embedding_size=self.model.shared.embedding_dim
        self.out_channel=200
        self.slidding_windows=3
        self.layers=nn.Sequential(
            nn.Conv1d(self.embedding_size,self.out_channel,kernel_size=self.slidding_windows),
            nn.BatchNorm1d(self.out_channel),
            nn.GELU()            
        )
        self.fc=nn.Linear(self.out_channel,1)
        self.init_weights()
    def init_weights(self):
        nn.init.xavier_normal_(self.fc.weight)
        nn.init.xavier_normal_(self.proj_linear.weight)
        nn.init.constant_(self.fc.bias, 0)
        nn.init.constant_(self.proj_linear.bias,0)
    def compute_copy_loss(self,copy_score,copy_mask,mask_select):
        copy_score=copy_score.reshape(-1,1)
        copy_score=torch.cat([1-copy_score,copy_score],dim=-1)
        copy_mask=copy_mask.reshape(-1)
        mask_select=mask_select.reshape(-1)
        loss_fct=nn.CrossEntropyLoss(reduction='none')
        loss=loss_fct(copy_score,copy_mask)
        loss=loss.masked_select(mask_select)
        
        return loss.mean()
    def compute_copy_loss2(self,copy_score,copy_mask,mask_select):
        # copy_score=copy_score.reshape(-1,1)
        # copy_score=torch.cat([1-copy_score,copy_score],dim=-1)
        # copy_mask=copy_mask.reshape(-1)
        # mask_select=mask_select.reshape(-1)
        # loss_fct=nn.CrossEntropyLoss(reduction='none')
        # loss=loss_fct(copy_score,copy_mask)
        copy_prob = torch.where(copy_mask.bool(), 1 - copy_score, copy_score)
        loss=copy_score.masked_select(mask_select).mean()
        
        return loss
    def forward(self, input_ids, attention_mask=None, encoder_outputs=None,
            decoder_input_ids=None, decoder_attention_mask=None, input_node_ids=None, input_edge_ids=None,
            node_length=None, edge_length=None, adj_matrix=None, decoder_whole_ids=None, decoder_cached_states=None,
            use_cache=False,use_pos=None, copy_mask_ids=None,copy_mask_ids_attn=None,windows=None, use_seman=True,is_training=False):
        ...
        # print('sshapK:',gen_out.shape,decoder_input_ids.shape,use_pos.shape)

        if is_training:
            
            word_embedding=self.model.shared(decoder_input_ids)
            # use_pos=use_pos.unsqueeze(1)
            pos_all=use_pos.repeat(1,int(word_embedding.shape[1]/use_pos.shape[1]),1)
            # print('刷出去：',gen_out.shape,pos_all.shape,word_embedding.shape)
            copy_prob = torch.sigmoid(self.proj_linear(torch.cat([word_embedding, gen_out,pos_all], dim=-1))).squeeze(-1)
            #设置semantic
            if use_seman:
                sem_embedding=self.model.shared(windows)
                new_padding=torch.cat([sem_embedding,outputs[0]],dim=1)
                new_padding=new_padding.permute(0,2,1) #conv1d shape
                new_padding=self.layers(new_padding)
                new_padding=self.fc(new_padding.permute(0,2,1))
                new_padding=torch.relu(new_padding)
                new_padding=new_padding.squeeze(-1)
                copy_prob=new_padding*self.coefficient+(1-self.coefficient)*copy_prob
            copy_loss=self.compute_copy_loss(copy_prob,copy_mask_ids,copy_mask_ids_attn)
        lm_logits = F.linear(outputs[0], self.model.shared.weight, bias=self.final_logits_bias)
        if is_training:
            ...
            return loss+0.3*copy_loss
        return (lm_logits, ) + outputs[1:],0
